...
- Detached HEAD behavior: if `--open-pr` is run when no branch is checked out, should the CLI fail immediately?
- Branch numbering strategy: when creating `repo-conventions-2`, `repo-conventions-3`, and so on, should the CLI choose the next available name by checking local branches only, or both local and remote branches?
- Convention naming fallback: for the commit message `Apply convention <name>.`, if the convention is at the repository root and the name cannot be inferred cleanly from the path, what exact fallback name should be used?
